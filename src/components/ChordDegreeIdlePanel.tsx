...
type SettingsProps = Pick<Props, 'selectedKey' | 'range' | 'inversionMode' | 'onKeyChange' | 'onRangeChange' | 'onInversionModeChange' | 'onApplyPreset'> & { title: string }

function ChordDegreeSettings({ title, selectedKey, range, inversionMode, onKeyChange, onRangeChange, onInversionModeChange, onApplyPreset }: SettingsProps) {
  return <div className="grid gap-4">
    ...
    <div className="grid gap-4 rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-elevated)] p-4">
      ...
      <div><p className="mb-2 text-xs font-medium text-[var(--text-secondary)]">出题范围</p><SegmentedControl value={range} onChange={onRangeChange} options={[{ value: 'primary', label: '1/4/5' }, { value: 'common', label: '+ 6' }, { value: 'major-key', label: '1–6' }, { value: 'all', label: '1–7' }]} /></div>
...
export function ChordDegreeIdlePanel({ quiz, sessionStats, history, sessionCompleted, selectedKey, range, inversionMode, onKeyChange, onRangeChange, onInversionModeChange, onApplyPreset, onPlayDo }: Props) {
  ...
  if (quiz && total > 0) return <PlayAreaCard className="gap-6">
    ...
    <ChordDegreeSettings title="下一局设置" selectedKey={selectedKey} range={range} inversionMode={inversionMode} onKeyChange={onKeyChange} onRangeChange={onRangeChange} onInversionModeChange={onInversionModeChange} onApplyPreset={onApplyPreset} />
  ...
  return <PlayAreaCard className="gap-6">
    ...
    <ChordDegreeSettings title="一键难度" selectedKey={selectedKey} range={range} inversionMode={inversionMode} onKeyChange={onKeyChange} onRangeChange={onRangeChange} onInversionModeChange={onInversionModeChange} onApplyPreset={onApplyPreset} />
    <Button type="button" variant="ghost" onClick={onPlayDo}>♪ 播放 do</Button>
    <ChordDegreeErrorChart sessionStats={sessionStats} history={history} />
  </PlayAreaCard>
}
